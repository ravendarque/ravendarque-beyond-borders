/**
 * E2E test to verify zoom creates visually different rendered images
 * Compares blob URLs at 0% vs 10% zoom to confirm zoom is actually applied
 */
...
test.describe('Zoom Comparison Final', () => {
  test('should render different images at 0% vs 10% zoom', async ({ page }) => {
    ...
    const zoomSlider0 = page.getByLabel('Zoom level').locator('[role="slider"]').first();
    await zoomSlider0.focus();
    ...
    // Set zoom to 10%
    const zoomSlider10 = page.getByLabel('Zoom level').locator('[role="slider"]').first();
    await zoomSlider10.focus();
    await page.keyboard.press('Home');
    for (let i = 0; i < 10; i++) {
    ...
    expect(src10).toContain('blob:');
    
    // CRITICAL: The images MUST be different if zoom is working
    // Different blob URLs means different rendering occurred
    ...
    console.log('✓ Zoom is being applied correctly!');
  });
});
